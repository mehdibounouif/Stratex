...
class DataCleaner:
    ...
    def validate_stock_data(self, df, ticker=None):
        """
        Validate stock data quality. Returns a dict with data_quality_score (0-100),
        issues list, null_counts, large_price_gaps, volume_anomalies.
        """
        label = ticker or "Unknown"
        report = {
            'ticker': label,
        ...
        if date_col:
            try:
                ...
                date_range = pd.bdate_range(df[date_col].min(), df[date_col].max())
        ...
        if close_col:
            try:
                ...
                gaps = pct_changes[pct_changes > 0.10]
                report['large_price_gaps'] = list(gaps.index)
                if len(gaps) > 0:
                    report['issues'].append(f"{len(gaps)} large price gaps >10%")
                    report['data_quality_score'] -= min(20, len(gaps) * 2)
            except Exception:
                pass
        ...
        if vol_col:
            try:
                ...
                zero_vol = int((volumes <= 0).sum())
                report['volume_anomalies'] = zero_vol
                if zero_vol > 0:
                    report['issues'].append(f"{zero_vol} zero/negative volume days")
                    report['data_quality_score'] -= min(10, zero_vol)
            except Exception:
                pass

        report['data_quality_score'] = max(0.0, round(report['data_quality_score'], 1))
        return report
